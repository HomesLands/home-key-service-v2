...
import { constructors, hooks, methods, models, plugins, schema, virtuals, isFirstInit } from './data';
...
export class Typegoose {
  ...
  private buildSchema<T>(t: T, name: string, schemaOptions: any, sch?: mongoose.Schema) {
    ...
    if (!sch) {
      ...
      // If schema still be not init, create the new one
      if(!isFirstInit[name]){
          sch = schemaOptions ? new Schema(schema[name], schemaOptions) : new Schema(schema[name]);
          isFirstInit[name] = true;
      }else{ // Else, it the existing one
          sch = mongoose.model(name).schema;
      }
